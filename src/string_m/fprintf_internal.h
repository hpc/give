...
static int ccomputeLen(string_m format, va_list ap, size_t *total_width, int *strtype){
	...
	*strtype = STRTYPE_NTBS;
	...
	retValue = cgetstr_m(format, &porig);
	if (retValue) {
	  return retValue;
	}
	...
	retValue = strlen_m(format, total_width);
	if (retValue) {
	     free(porig);
	     return EINVAL;
	}  
...
wcomputeLen(string_m format, va_list ap, size_t *total_width, int *strtype){
	...
	retValue = wgetstr_m(format, &porig);
	if (retValue) {
	  return retValue;
	}
				...
				case 's':
					{
						...
						retValue = cgetstr_m(s, &str);
						if (retValue){ free(porig);
							return EINVAL;}
						if (str) {
							char *ptr = str;
							while (*ptr) {
