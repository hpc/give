			...
			case 's':
				{
					...
					if (field_width == -1) field_width =  SIZE_MAX;
					...
					if (qualifier == 'l'){
						...
						retValue = wstrcpy_m(*s, wtmp);
						free(wtmp);
						free(tmp);
						if (retValue) return retValue;
					} else {
						retValue = cstrcpy_m(*s, tmp);
						free(tmp);
						if (retValue) return retValue;
					}
					num++;
			...
			case 's':
				{
					...
					if (qualifier == L'h'){
						...
						retValue = cstrcpy_m(*s, ctmp);
						free(ctmp);
						free(tmp);
						if (retValue) return retValue;
					} else {
						retValue = wstrcpy_m(*s, tmp);
						free(tmp);
						if (retValue) return retValue;
					}
					num++;
