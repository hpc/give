...
#define FUNC vsprintf_m
	...
	if (format->strtype == STRTYPE_WSTR) {
		...
		retValue = wcomputeLen(format, ap, &total_width, &strtype);
	} else if (format->strtype == STRTYPE_NTBS) {
		if (format->str.cstr == NULL) {
			return cstrcpy_m(s, NULL);
		}
		retValue = ccomputeLen(format, ap, &total_width, &strtype);
	} else {
	...
	if (strtype == STRTYPE_NTBS) {
		retValue = coutputString(s, format, args);
	} else if (strtype == STRTYPE_WSTR) {
		retValue = woutputString(s, format, args);
	} else {	
		ERROR(EINVAL);
	}
	if (retValue) {
	...
	retValue = strlen_m(s, (size_t *)count);
