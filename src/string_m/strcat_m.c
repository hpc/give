...
//          s->str  - Any string is valid ("" is valid, NULL is not valid)
errno_t 
wstrcat_m(string_m s1, wchar_t *s2) {
    string_m const ostr = s1;
    errno_t rv;
    size_t olen, ilen;
...
cstrncat_m(string_m s1, const char *s2, rsize_t nchar){
    ...
    if (!ostr->size) return cstrncpy_m(s1, strdup(s2), nchar);
	if (!s2) return 0;
	...
	if ( (s1->charset.cstr) && (strspn(s2, s1->charset.cstr) < (ilen-1)) ){
		ErrorHandler("cstrcat_m: Charset Invalidates string", strdup(s2), EINVAL);
		return EINVAL;
	}
...
wstrncat_m(string_m s1, const wchar_t *s2, rsize_t nchar){
    ...
    if (!ostr->size) return wstrncpy_m(s1, wcsdup(s2), nchar);
	if (!s2) return 0;
	...
	if ( (s1->charset.wstr) && (wcsspn(s2, s1->charset.wstr) < (ilen-1)) ){
		ErrorHandler("cstrcat_m: Charset Invalidates string", wcsdup(s2), EINVAL);
		return EINVAL;
	}
