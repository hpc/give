...
cstrcpy_m(string_m s1, char *s2) {
    ...
    if (s1->strtype == STRTYPE_WSTR) {   	
        if (s1->str.wstr) free(s1->str.wstr);
        s1->str.cstr = NULL;
        s1->size = 0;
        s1->strtype = STRTYPE_NTBS;
		if (s1->charset.wstr){
			wchar_t *t = (wchar_t *)malloc(wcslen(s1->charset.wstr));
			wcscpy(t, s1->charset.wstr);
			free(s1->charset.wstr);
			s1->charset.cstr = (char *)malloc(wcslen(t)+1);
			wcstombs(s1->charset.cstr, t, wcslen(t));
		}
    }
...
//          s->str  - Any string is valid ("" is valid, NULL is not valid)
errno_t
wstrcpy_m(string_m s1, wchar_t *s2) {
    size_t s2_size;
    int rv;
    //validate arguments
	if (!s1){
...
//          s->str  - Any string is valid ("" is valid, NULL is not valid)
errno_t
cstrncpy_m(string_m s1, char *s2, size_t nchar) {
    size_t s2_size;
...
//          s->str  - Any string is valid ("" is valid, NULL is not valid)
errno_t
wstrncpy_m(string_m s1, wchar_t *s2, size_t nchar) {
    size_t s2_size;
    int rv;
    //validate arguments
	if (!s1){
