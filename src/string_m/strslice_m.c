...
cstrslice_m(string_m s1, char *cstr, rsize_t offset, rsize_t len){
	...
	if(s1->strtype == STRTYPE_WSTR){
		wchar_t *t = (wchar_t *)calloc(strlen(cstr)+1, sizeof(wchar_t));
		mbstowcs( t,cstr, strlen(cstr));
	...
	rv = strlen_m(s1, &s1_size);
	if(rv != 0) ERROR(rv);
	
	if(!cstr || offset > (s2_size = strlen(cstr))){
		rv = cstrcpy_m(s1, NULL);
		return rv;
	}
...
wstrslice_m(string_m s1, wchar_t *wstr, rsize_t offset, rsize_t len){
	...
	if(s1->size < (ilen + s1_size)*sizeof(wchar_t)){
...
cstrleft_m(string_m s1, char *cstr, rsize_t len){
	...
	if(s1->strtype == STRTYPE_WSTR){
		wchar_t *t = (wchar_t *)calloc(s2_size, sizeof(wchar_t));
		mbstowcs(t, cstr, s2_size);
