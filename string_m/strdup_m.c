...
errno_t strcreate_m(string_m *s, const char *cstr, const size_t maxsize, const char *charset) {
	...
	if(charset && strlen(charset) == 0){ 
		ErrorHandler("strcreate_m: 4th Argument May Not Be An Empty String", strdup(charset), EINVAL);
		ERROR(EINVAL);
	}
...
errno_t wstrcreate_m(string_m *s, const wchar_t *wstr, const size_t maxsize, const wchar_t *charset) {
	...
	if(charset && wcslen(charset) == 0){
		ErrorHandler("wstrcreate_m: 4th Argument May Not Be An Empty String", wcsdup(charset), EINVAL);
		ERROR(EINVAL);
	}
    *s = (string_m)calloc(1, sizeof(struct string_mx));
    ...
    rv = wmakestr(*s, wstr);
    if (rv) { free(s); *s = NULL; ERROR(rv); }

    (*s)->maxsize = maxsize;
    if (charset) {
    	size_t nchar = wcslen(charset);
    	(*s)->charset.wstr = malloc((nchar+1)*sizeof(wchar_t));
	if (!(*s)->charset.wstr) {
		if ((*s)->str.wstr) free((*s)->str.wstr);
		free(*s);
		*s = NULL;
		ErrorHandler("wstrcreate_m: Malloc Error", s, ENOMEM);
		ERROR(ENOMEM);
	}
	wcsncpy((*s)->charset.wstr, charset, nchar+1);
